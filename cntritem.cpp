...
BOOL CDrawItem::OnChangeItemPosition(const CRect& rectPos)
{
	...
	if (rect != m_pDrawObj->m_rectPos)
	{
		// invalidate old rectangle
		m_pDrawObj->Invalidate();

		// update to new rectangle
		m_pDrawObj->m_rectPos = rect;
...
void CDrawItem::OnGetItemPosition(CRect& rectPos)
{
	ASSERT_VALID(this);

	// update to extent of item if m_rectPos is not initialized
	if (m_pDrawObj->m_rectPos.IsRectEmpty())
		UpdateExtent();

	// copy m_rectPos, which is in document coordinates
	CDrawView* pView = GetActiveView();
	ASSERT_VALID(pView);
	rectPos = m_pDrawObj->m_rectPos;
	pView->DocToClient(rectPos);
}
...
BOOL CDrawItem::UpdateExtent()
{
	...
	{
		m_pDrawObj->m_rectPos.right =
			m_pDrawObj->m_rectPos.left + MulDiv(size.cx, 10, 254);
		m_pDrawObj->m_rectPos.bottom =
			m_pDrawObj->m_rectPos.top - MulDiv(size.cy, 10, 254);
	}
	// else data changed so scale up rect as well
	else if (!IsInPlaceActive() && size != m_pDrawObj->m_extent)
	{
		m_pDrawObj->m_rectPos.right = m_pDrawObj->m_rectPos.left +
			MulDiv(m_pDrawObj->m_rectPos.Width(), size.cx, m_pDrawObj->m_extent.cx);
		m_pDrawObj->m_rectPos.bottom = m_pDrawObj->m_rectPos.top +
			MulDiv(m_pDrawObj->m_rectPos.Height(), size.cy, m_pDrawObj->m_extent.cy);
	}
